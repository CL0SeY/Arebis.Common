...
namespace Arebis.Logging.GrayLog.NetCore
{
    ...
    {
        /// TODO: How should configuration be handled? DI?
        /*
        /// <summary>
        ...
        */
        ...
        public GrayLogUdpClient(string facility, string host, int port = 12201)
            : this(facility, new UdpClient(AddressFamily.InterNetwork), true)
        {
            this.Port = port;
            this.Host = host;
        }
            ...
            : base(facility)
        {
            ...
            this.MaxPacketSize = 512; // Int32.Parse(ConfigurationManager.AppSettings["GrayLogUdpMaxPacketSize"] ?? "512");
        }
        ...
        protected override async Task InternallySendMessageAsync(byte[] messageBody)
        {
            // Get message id:
            ...
            using (var objectStream = new MemoryStream(messageBody))
            {
                ...
                for (byte chunkNumber = 0; chunkNumber < chunkCount; chunkNumber++)
                {
                    ...
                    await this.UdpClient.SendAsync(chunkBuffer, chunkSize, this.Host, this.Port);
                }
            }
        }
